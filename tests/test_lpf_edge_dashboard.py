...
class LPFEdgeDashboardTests(unittest.TestCase):
    ...
    def test_streamlit_page_enables_only_controlled_daily_actions(self) -> None:
        ...
        self.assertIn("publication.market_settlement_sha256", source)
        ...
        self.assertIn("puis les cotes françaises sont réutilisées", source)
        self.assertIn("publication.odds.status.value", source)
        ...
        self.assertIn("publication.prediction.daily_selection_sha256", source)
        self.assertIn("publication.prediction.daily_selection_count", source)
        self.assertIn("load_daily_selection", source)
        self.assertIn("LPFEdgeDailySelectionError", source)
        self.assertIn("Matchs retenus pour les pronostics", source)
        self.assertIn("Aucun prono retenu aujourd’hui", source)
        self.assertIn("Principal", source)
        self.assertIn("Secondaire", source)
        self.assertIn("Mode observation", source)
        self.assertIn("ne déclenche aucune mise réelle", source)
        self.assertIn("build_evaluation_supervision", source)
        self.assertIn("LPFEdgeEvaluationSupervisionError", source)
        self.assertIn("EvaluationDayEvidence", source)
        self.assertIn("Supervision de l’évaluation", source)
        self.assertIn("Shadow v2 — toutes les prédictions", source)
        self.assertIn("Sélecteur de pronostics — mode observation", source)
        self.assertIn("Recul statistique minimal", source)
        self.assertIn("Journées sans prono", source)
        self.assertIn("Baisse maximale théorique", source)
        self.assertIn("Pire série de défaites", source)
        self.assertIn("Il ne valide ni la rentabilité", source)
        ...
        self.assertIn("Cotes françaises — Moneyline", source)
        self.assertIn("région française", source)
        self.assertIn("Betclic, NetBet, PMU", source)
        self.assertIn("odds_region_label", source)
        self.assertIn("build_french_market_comparison", source)
        self.assertIn("LPFEdgeMarketComparisonError", source)
        self.assertIn("required_region=\"fr\"", source)
        self.assertIn("completed_at_or_before_utc=day.certified_at_utc", source)
        self.assertIn("Comparaison LPF Edge / marché français", source)
        self.assertIn("toute collecte ultérieure est ignorée", source)
        self.assertIn("Collecte française figée", source)
        self.assertIn("Marché français corrigé", source)
        self.assertIn("Écart LPF – marché", source)
        self.assertIn("Meilleure cote française", source)
        self.assertIn("Il ne garantit ni victoire ni rentabilité", source)
        self.assertIn("build_market_evaluation_history", source)
        self.assertIn("LPFEdgeMarketEvaluationError", source)
        self.assertIn("Évaluation historique des écarts LPF–marché", source)
        self.assertIn("Verdict prospectif LPF/marché", source)
        self.assertIn("load_market_settlement", source)
        self.assertIn("Résultat théorique scellé", source)
        self.assertIn("Verdict des matchs retenus", source)
        self.assertIn("sealed_market_settlement.selected_evaluated_count", source)
        self.assertIn("sealed_market_settlement.selected_theoretical_roi_percent", source)
        self.assertIn("d’origine n’est jamais modifié", source)
        self.assertIn("Observations évaluées", source)
        self.assertIn("Résultat théorique", source)
        self.assertIn("Rendement théorique", source)
        self.assertIn("Sans cote antérieure", source)
        self.assertIn("limité pour tirer une conclusion fiable", source)
        self.assertIn("aucune mise réelle n’est effectuée", source)
        self.assertIn("il ne constitue ni un gain réel", source)
        self.assertIn("if band.evaluated_count > 0", source)
        ...
        self.assertIn("lpf_edge_odds_success", source)
        self.assertIn("Lanceurs annoncés", source)
        self.assertIn("Limite Shadow v2 (Paris)", source)
        self.assertIn("Lanceur extérieur", source)
        self.assertIn("Lanceur domicile", source)
        self.assertIn("Non annoncé", source)
