...
class LPFEdgeDashboardTests(unittest.TestCase):
    ...
    def test_streamlit_pages_separate_actions_history_and_statistics(self) -> None:
        root = Path(__file__).parents[1]
        today = root.joinpath("pages", "1_LPF_Edge.py").read_text(encoding="utf-8")
        history = root.joinpath("pages", "2_Historique_MLB.py").read_text(
            encoding="utf-8"
        )
        statistics = root.joinpath("pages", "3_Statistiques_MLB.py").read_text(
            encoding="utf-8"
        )
        backups = root.joinpath("pages", "4_Sauvegardes.py").read_text(
            encoding="utf-8"
        )
        combined = "\n".join((today, history, statistics, backups))

        self.assertIn("Centre d’actions quotidien", today)
        self.assertIn("execute_afternoon_prediction_routine", today)
        self.assertIn("execute_daily_results_publication", today)
        self.assertIn("execute_daily_odds_collection", today)
        self.assertIn("Préparation avant les prédictions", today)
        self.assertIn("Dernières cotes enregistrées", today)
        self.assertIn("daily.morning_action.label", today)
        self.assertIn("daily.afternoon_action.label", today)
        self.assertIn("Matin — Résultats", today)
        self.assertIn("Après-midi — Prédictions", today)
        self.assertIn("puis les cotes françaises sont réutilisées", today)
        self.assertIn("publication.prediction.market_snapshot_sha256", today)
        self.assertIn("publication.prediction.daily_selection_sha256", today)
        self.assertIn("publication.market_settlement_sha256", today)
        self.assertIn("Pronostics du jour", today)
        self.assertIn("load_daily_selection", today)
        self.assertIn("Choix principal", today)
        self.assertNotIn("Journée à consulter", today)
        self.assertNotIn("Supervision de l’évaluation", today)
        self.assertNotIn("Historique des sauvegardes", today)

        self.assertIn("Journée à consulter", history)
        self.assertIn("Matchs retenus pour les pronostics", history)
        self.assertIn("Aucun prono retenu aujourd’hui", history)
        self.assertIn("Principal", history)
        self.assertIn("Secondaire", history)
        self.assertIn("Comparaison LPF Edge / marché français", history)
        self.assertIn('required_region="fr"', history)
        self.assertIn("completed_at_or_before_utc=day.certified_at_utc", history)
        self.assertIn("toute collecte ultérieure est ignorée", history)
        self.assertIn("Résultats vérifiés", history)
        self.assertIn("Verdict prospectif LPF/marché", history)
        self.assertIn("Score final (dom. – ext.)", history)
        self.assertNotIn("Score final (ext. – dom.)", history)
        self.assertIn('f"{home_name} vs {away_name}"', history)
        self.assertNotIn('f"{away_name} @ {home_name}"', history)
        self.assertNotIn("st.button(", history)

        self.assertIn("Supervision de l’évaluation", statistics)
        self.assertIn("build_evaluation_supervision", statistics)
        self.assertIn("Shadow v2 — toutes les prédictions", statistics)
        self.assertIn("Sélecteur de pronostics — mode observation", statistics)
        self.assertIn("Recul statistique minimal", statistics)
        self.assertIn("Journées sans prono", statistics)
        self.assertIn("Baisse maximale théorique", statistics)
        self.assertIn("Pire série de défaites", statistics)
        self.assertIn("Évaluation historique des écarts LPF–marché", statistics)
        self.assertIn("build_market_evaluation_history", statistics)
        self.assertIn("limité pour tirer une conclusion fiable", statistics)
        self.assertNotIn("st.button(", statistics)

        self.assertIn("execute_verified_local_backup", backups)
        self.assertIn("Historique des sauvegardes", backups)
        self.assertIn("list_local_backup_names", backups)
        self.assertIn("load_verified_local_backup", backups)
        self.assertIn("selected_backup.archive_bytes", backups)
        self.assertIn('key="download_existing_backup"', backups)

        self.assertNotIn("run_schedule_ingestion", combined)
        self.assertNotIn("shadow_prediction", combined)
        self.assertNotIn("shadow_certification", combined)
        self.assertNotIn("shadow_scoring", combined)
        self.assertIn('height="content"', combined)
